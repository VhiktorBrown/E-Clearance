package com.theelitedevelopers.e_clearance.modules.student_affairs;
...
public class StudentAffairsActivity extends AppCompatActivity {
    ActivityStudentAffairsBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityStudentAffairsBinding.inflate(getLayoutInflater());
        setContentView(binding.getRoot());

        // populate layout field
        view_list.add(findViewById(R.id.lyt_pay_for_nysc_card));
        view_list.add(findViewById(R.id.lyt_download_temporal_card));

        // populate view step (circle in left)
        step_view_list.add(((RelativeLayout) findViewById(R.id.step_pay_for_nysc_card)));
        step_view_list.add(((RelativeLayout) findViewById(R.id.step_download_temporal_card)));
        ...
        binding.btPayForNyscCard.setOnClickListener(v -> {
            index = 0;
    ...
    private void mockHandler(int index){
        ...
        handler.postDelayed(() -> {
            if(index == 1){
                AppUtils.displayToast(getApplicationContext(), "NYSC Temporal ID Card downloaded successfully.");
    ...
    public void clickLabel(View view) {
        int id = view.getId();
        switch (id) {
            case R.id.pay_for_nysc_card_title:
    ...
    private void markResultClearanceAsCompleted(String uid, boolean status){
        ...
        clearanceProgress.put("completedUploadForStudentAffairs", status);
        ...
        database.collection(Constants.UPLOADED)
                ...
                .addOnSuccessListener(unused -> {
                    if(status){
                        Toast.makeText(getApplicationContext(), Constants.STUDENT_AFFAIRS_CLEARANCE_COMPLETED, Toast.LENGTH_SHORT).show();
                    }
                })
                ...
                .addOnSuccessListener(documentSnapshot -> {
                    if(documentSnapshot.exists()){
                        ...
                        if(completedUpload != null){
                            if(completedUpload.getCompletedUploadForStudentAffairs() != null &&
                                    completedUpload.getCompletedUploadForStudentAffairs()){
